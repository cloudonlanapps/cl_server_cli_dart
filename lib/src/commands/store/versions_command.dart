...
/// Execute store versions command
Future<void> runStoreVersionsCommand(
  ...
  try {
    final result = await manager.getVersions(entityId!);

    if (!result.isSuccess || result.data == null) {
      outputError(context, result.error ?? 'Failed to get versions');
    }

    outputSdkList(context, result.data!);
  } finally {
